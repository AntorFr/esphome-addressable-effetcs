...
async def addressable_stars_effect_to_code(config, effect_id):
    ...
    color = cg.StructInitializer(
                ...
                ("r", int(round(config[CONF_RED] * 255))),
                ("g", int(round(config[CONF_GREEN] * 255))),
                ("b", int(round(config[CONF_BLUE] * 255))),
                ("w", int(round(config[CONF_WHITE] * 255))),
                ("brightness", int(round(config[CONF_BRIGHTNESS] * 100))),
            )
